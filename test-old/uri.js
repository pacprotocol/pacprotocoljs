...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    uri.address.should.equal('Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    expect(uri.amount).to.be.undefined;
    expect(uri.otherParam).to.be.undefined;

    uri = URI.parse('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=123.22');
    uri.address.should.equal('Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG').should.equal(true);
    URI.isValid('pac:yRM2dX5HJyvbFaGubkVPio9W6Y8ELDnJrm').should.equal(true);

    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2')
                .should.equal(true);
    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&other=param')
                .should.equal(true);
    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&req-other=param',
                ['req-other']).should.equal(true);
    URI.isValid('pac:ySfKnKxK4S6TkVg6kyUEdXsoLPoCe4YdVp?amount=0.1&' +
                'r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu').should.equal(true);

    URI.isValid('pac:').should.equal(false);
    URI.isValid('pac:badUri').should.equal(false);
    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPH?amount=bad').should.equal(false);
    URI.isValid('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPH?amount=1.2&req-other=param')
                .should.equal(false);
    URI.isValid('pac:?r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu')
                .should.equal(false);
  });
  ...
  it('do not need new keyword', function() {
    var uri = URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  ...
  describe('instantiation from bitcoin uri', function() {
    ...
    it('parses address', function() {
      uri = new URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=123.22');
    ...
    it('parses a testnet address', function() {
      uri = new URI('pac:yRM2dX5HJyvbFaGubkVPio9W6Y8ELDnJrm');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&other=param');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('pac://Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
    uri.address.toString().should.equal('Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');
  });

  it('should input/output String', function() {
    var str = 'pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG'
    );
    ...
    new URI({
      ...
      'pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG?amount=1.10001&message=Hello%20World&something=else'
    );
  ...
  it('should be case insensitive to protocol', function() {
    ...
    var uri2 = new URI('pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG');

    uri1.address.toString().should.equal(uri2.address.toString());
  });

  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'pac:ySfKnKxK4S6TkVg6kyUEdXsoLPoCe4YdVp?amount=0.1&' +
  ...
  it('displays nicely on the console (#inspect)', function() {
    var uri = 'pac:Xo4vyw1FtA88rYPYjbNT9kwhVokHHsSuPG';
