# Transaction
Dashcore provides a very simple API for creating transactions. We expect this API to be accessible for developers without knowing the working internals of PAC Protocol in deep detail. What follows is a small introduction to transactions with some basic knowledge required to use this API.
...
Note also that an output can be used only once. That's why there's a concept of "change address" in the PAC Protocol ecosystem: if an output of 10 PAC is available for me to spend, but I only need to transmit 1 PAC, I'll create a transaction with two outputs, one with 1 PAC that I want to spend, and the other with 9 PAC to a change address, so I can spend this 9 PAC with another private key that I own.
...
```bash
pacprotocol-cli sendrawtransaction <serialized transaction>
```
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 PAC
```
...
- `script`: the `Script` instance for this input. Usually called `scriptSig` in the PAC Protocol community.
...
- `toBuffer`: Serializes the transaction for sending over the wire in the PAC Protocol network
...
All PAC Protocol transactions contain a locktime field. The locktime indicates the earliest time a transaction can be added to the blockchain. Locktime allows signers to create time-locked transactions which will only become valid in the future, giving the signers a chance to change their minds. Locktime can be set in the form of a PAC Protocol block height (the transaction can only be included in a block with a higher height than specified) or a Linux timestamp (transaction can only be confirmed after that time). For more information see [Bitcoin's development guide section on locktime](https://bitcoin.org/en/developer-guide#locktime-and-sequence-number).
