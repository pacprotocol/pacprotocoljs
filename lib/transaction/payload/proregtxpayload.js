...
function ProRegTxPayload(options) {
  ...
  if (options) {
    ...
    this.keyIDOwner = Address(options.ownerAddress).hashBuffer.reverse().toString('hex');
    this.pubKeyOperator = options.pubKeyOperator;
    this.keyIDVoting = Address(options.votingAddress).hashBuffer.reverse().toString('hex');
...
ProRegTxPayload.calculateInputsHash = AbstractPayload.calculateInputsHash;
...
ProRegTxPayload.prototype.toJSON = function toJSON(options) {
  ...
  var network = this.network || (options && options.network);
  this.validate();
  var payloadJSON = {
 ...
 */
ProRegTxPayload.prototype.sign = function(privateKey) {
  var s = new Script(this.scriptPayout).toAddress(this.network).toString() + '|';
  s += this.operatorReward + '|';
  s += Address.fromPublicKeyHash(Buffer.from(this.keyIDOwner, 'hex').reverse(), this.network) + '|';
  s += Address.fromPublicKeyHash(Buffer.from(this.keyIDVoting, 'hex').reverse(), this.network) + '|';
  s += Buffer.from(this.getHash({skipSignature: true})).reverse().toString('hex');

  var signatureBuffer = Signer.signHash(Message(s, this.network).magicHash(), privateKey);
 ...
 * @return {boolean}
 */
ProRegTxPayload.prototype.verifySignature = function (publicKeyId) {
  var s = new Script(this.scriptPayout).toAddress(this.network).toString() + '|';
  s += this.operatorReward + '|';
  s += Address.fromPublicKeyHash(Buffer.from(this.keyIDOwner, 'hex').reverse(), this.network) + '|';
  s += Address.fromPublicKeyHash(Buffer.from(this.keyIDVoting, 'hex').reverse(), this.network) + '|';
  ...
  return Signer.verifyHashSignature(Message(s, this.network).magicHash(), signatureBuffer, publicKeyId);
};
