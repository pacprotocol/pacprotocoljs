 ...
 * @returns {Message}
 */
var Message = function Message(message, network) {
  if (!(this instanceof Message)) {
    return new Message(message, network);
  }
  $.checkArgument(_.isString(message), 'First argument should be a string');
  this.message = message;
  this.MAGIC_BYTES = Buffer.from((network || networks.defaultNetwork).messagePrefix);
...
Message.prototype.magicHash = function magicHash() {
  var prefix1 = BufferWriter.varintBufNum(this.MAGIC_BYTES.length);
  var messageBuffer = Buffer.from(this.message);
  var prefix2 = BufferWriter.varintBufNum(messageBuffer.length);
  var buf = Buffer.concat([prefix1, this.MAGIC_BYTES, prefix2, messageBuffer]);
